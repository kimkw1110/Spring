...
public class MainClass {
	public static void main(String[] args) {
      ...
      new GenericXmlApplicationContext(
     		 "classpath:appCtxImport.xml");

		
		//샘플데이터
		InitSampleData  initSampleData = 
				ctx.getBean("initSampleData" , 
		...
		registerService = ctx.getBean("studentRegisterService" ,
				StudentRegisterService.class );
		registerService.register( new Student("hbs006",
	"dear" , "p0006" , "melissa", 26, 'W', "Music") );
		...
		StudentModifyService modifyService = 
				ctx.getBean("studentModifyService", 
				StudentModifyService.class);
		modifyService.modify( new Student("hbs006","pig",
			"p0066", "melissa", 27,  'W', "Computer"));
		
		psi.printStudentInfo();	
		
		//특정 학번에 해당하는 학생을 삭제하고 출력
		StudentDeleteService deleteService = 
				ctx.getBean("studentDeleteService" , 
